...
namespace Ideal.Platform.Common.Snowflake
{
    ...
    public class SnowFlakeBase
    {
        ...
        private static readonly Lazy<Snowflake> lazy =
            new Lazy<Snowflake>(() => new Snowflake());
        public static Snowflake Instance { get { return lazy.Value; } }
        private void Init()
        {
            Snowflakes(0L, -1);
        }
        ...
        public void Init(long machineId, long datacenterId)
        {
        ...
        private void Snowflakes(long machineId, long datacenterId)
        {
            if (machineId >= 0)
            {
                ...
                SnowFlakeBase.machineId = machineId;
            }
            if (datacenterId >= 0)
            {
                ...
                SnowFlakeBase.datacenterId = datacenterId;
            }
        }
        ...
        public long GetId()
        {
            ...
            {
                long timestamp = GetTimestamp();
                if (SnowFlakeBase.lastTimestamp == timestamp)
                { //同一微妙中生成ID
                    sequence = (sequence + 1) & sequenceMask; //用&运算计算该微秒内产生的计数是否已经到达上限
                    if (sequence == 0)
                    {
                        //一微妙内产生的ID计数已达上限，等待下一微妙
                        timestamp = GetNextTimestamp(SnowFlakeBase.lastTimestamp);
                    }
                }
                else
                {
                ...
                SnowFlakeBase.lastTimestamp = timestamp; //把当前时间戳保存为最后生成ID的时间戳
